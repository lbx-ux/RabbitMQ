package com.study.lab5.consumer.service;
 ...
 *    - 生产者确认失败会重发（lab4 的补偿任务就干这事）
 *    - 消费者 nack 后消息重新入队会再次投递
 *    - 消费者处理到一半宕机，MQ 会把 unacked 消息重新投给别人
 *  同一条业务消息可能到达 2~N 次，所以「加积分」「发短信」这类操作必须自己防重。
 *
 * 【企业级标准处理流程】（笔记 §4.3 的 7 步，SmsListener+本类实现它）
 ...
 *   5. 执行核心业务
 *   6. 提交事务
 *   7. 异常时回滚事务 + 【删除 Redis key】（否则这条消息永远无法重试了！）
 *
 * 【Redis + MySQL 双保险的分工】
...
public class IdempotentService {
    ...
    private static final String KEY_PREFIX = "lab5:idempotent:";
    /** 幂等 key 过期时间：24 小时（防 Redis 死锁 + 控制内存，笔记 §4.2 原话） */
     ...
     *
     * lab5_consumed_message.message_id 有 UNIQUE KEY：
     *   第一次插入成功 -> 返回 true
    ...
    public void releaseLock(String messageId) {
        ...
        log.info("[幂等-Redis] 业务失败/重放前，释放幂等 key {} -> deleted={}", key, deleted);
    }
}
